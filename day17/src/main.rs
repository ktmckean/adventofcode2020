...
fn update(grid: &mut  Vec::<Vec<Vec<i32>>>){
    ...
    for x in 0..grid.len(){
        for y in 0..grid.len(){
            for z in 0..grid.len() {
                let numActiveAdj = getNumAdjacent(grid, x as i64,y as i64,z as i64);
...
fn printGrid(grid: &Vec::<Vec<Vec<i32>>>){
    // assert!(grid != updated);
...
fn getNumAdjacent(grid: &Vec<Vec<Vec<i32>>>, x: i64, y: i64, z: i64) -> i32{
    let mut lowx :i64 = -1;
    let mut lowy :i64= -1;
    let mut highx:i64 = 1;
    let mut highy:i64 = 1;
